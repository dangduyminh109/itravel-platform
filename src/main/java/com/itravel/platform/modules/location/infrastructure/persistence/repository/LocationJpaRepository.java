...
public interface LocationJpaRepository extends JpaRepository<LocationJpaEntity, Long> {
    ...
    @Query(
            value = "SELECT u.* FROM location u " +
    ...
    Page<LocationJpaEntity> getLocations(@Param("keyword") String keyword, Pageable pageable, @Param("isDeleted") Boolean isDeleted,@Param("status") LocationStatus status);
    ...
    List<LocationJpaEntity> getTree(@Param("isDeleted") Boolean isDeleted, @Param("status") LocationStatus status);
}